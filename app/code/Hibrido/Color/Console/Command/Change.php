...
class Change extends Command
{
    ...
    protected function execute(
        ...
        foreach ($this->storeRepoManager->getList() as $store) {
            if($store->getId() === $id = $input->getArgument(self::ID_ARGUMENT)){
                $validator = true;
            }
        }   
        if(!$validator){
        ...
        if(ctype_xdigit($color) && strlen($color)==6){
            ...
            $output->writeln("ALTERADO COM SUCESSO PARA A COR: ".$color.' NA STORE VIEW '.$id.'.'.' ATUALIZE A PAGINA NO BROWSER!');
           
        }
        else{

            $output->writeln("FORMATO DE COR INVALIDO");
            $output->writeln("DIGITE A COR NO FORMATO HEX SEM #");
